(function () {
  ...
  const url =
    ...
    `&$expand=CyberSecurityCoE,CyberSecurityCoEBackup`;

  function getContainer() {
    return document.getElementById("cyberCoeContainer");
  }
  ...
  function showError(message) {
    ...
    container.innerHTML =
      '<p class="cyber-load-error" role="alert">' + message + "</p>";
  }
  ...
  const createProfilePic = (email) => {
    ...
    return `
      <div class="cyber-profile-pic-wrapper">
        <img src="${photoUrl}" alt="Profile" class="cyber-profile-pic"
  ...
  const formatName = (name, email) => {
    if (!name) {
      return `
        <div class="cyber-person-wrapper">
          <div class="cyber-name-text">TBD</div>
        </div>`;
    }
    ...
    const inner = `
      <div class="cyber-person-wrapper">
        ${pic}
        <div class="cyber-name-text">${displayName}</div>
      </div>`;
    return email
      ? `<a href="mailto:${email}" title="${email}" class="cyber-person-link">${inner}</a>`
      : inner;
  };
  ...
  async function loadTable() {
    ...
    try {
      ...
      let html = `<table class="cyber-table"><tbody>`;
      ...
      columns.forEach((item) => {
        const primary = getPerson(item.CyberSecurityCoE);
      ...
      columns.forEach((item) => {
        const backup = getPerson(item.CyberSecurityCoEBackup);
      ...
      console.error("Error fetching Cyber Security CoE items:", err);
      showError(
        err && err.message
          ? err.message
