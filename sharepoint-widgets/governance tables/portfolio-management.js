(function () {
  ...
  const url =
    ...
    `&$expand=PortfolioManagementCoE,PortfolioManagementCoEBackup`;

  function getContainer() {
    return document.getElementById("portfolioCoeContainer");
  }
  ...
  function showError(message) {
    ...
    container.innerHTML =
      '<p class="portfolio-load-error" role="alert">' + message + "</p>";
  }
  ...
  const createProfilePic = (email) => {
    ...
    return `
      <div class="portfolio-profile-pic-wrapper">
        <img src="${photoUrl}" alt="Profile" class="portfolio-profile-pic"
  ...
  const formatName = (name, email) => {
    if (!name) {
      return `
        <div class="portfolio-person-wrapper">
          <div class="portfolio-name-text">TBD</div>
        </div>`;
    }
    ...
    const inner = `
      <div class="portfolio-person-wrapper">
        ${pic}
        <div class="portfolio-name-text">${displayName}</div>
      </div>`;
    return email
      ? `<a href="mailto:${email}" title="${email}" class="portfolio-person-link">${inner}</a>`
      : inner;
  };
  ...
  async function loadTable() {
    ...
    try {
      ...
      let html = `<table class="portfolio-table"><tbody>`;
      ...
      columns.forEach((item) => {
        const primary = getPerson(item.PortfolioManagementCoE);
      ...
      columns.forEach((item) => {
        const backup = getPerson(item.PortfolioManagementCoEBackup);
      ...
      console.error("Error fetching Portfolio Management CoE items:", err);
      showError(
        err && err.message
          ? err.message
